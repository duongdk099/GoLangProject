...
func TestServiceGetAndUpdate(t *testing.T) {
	...
	if _, err := service.Get(context.Background(), 999); !errors.Is(err, httpapi.ErrNotFound) {
		t.Fatalf("Get(missing) error = %v, want not found", err)
	}
	if _, err := service.Get(context.Background(), 0); !errors.Is(err, httpapi.ErrValidation) {
		t.Fatalf("Get(invalid ID) error = %v, want validation", err)
	}
}

func TestServiceSetSkills(t *testing.T) {
	tests := []struct {
		name      string
		skills    []Skill
		want      []Skill
...
func TestServiceSkillAuthorizationAndLookups(t *testing.T) {
	store := newMemoryStore()
	user := seedUser(store, "Alice")
	service := NewService(store)

	if _, err := service.SetSkills(context.Background(), user.ID+1, user.ID, nil); !errors.Is(err, httpapi.ErrForbidden) {
		t.Fatalf("SetSkills() error = %v, want forbidden", err)
	}
	if _, err := service.SetSkills(context.Background(), 999, 999, nil); !errors.Is(err, httpapi.ErrNotFound) {
		t.Fatalf("SetSkills(missing) error = %v, want not found", err)
	}
	if _, err := service.Skills(context.Background(), 999); !errors.Is(err, httpapi.ErrNotFound) {
		t.Fatalf("Skills(missing) error = %v, want not found", err)
	}
	if _, err := service.Skills(context.Background(), 0); !errors.Is(err, httpapi.ErrValidation) {
