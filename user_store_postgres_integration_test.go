...
func TestPostgresUserStoreIntegration(t *testing.T) {
	...
	if _, err := store.GetByID(ctx, 2147483647); !errors.Is(err, ErrNotFound) {
		t.Fatalf("GetByID(missing) error = %v, want not found", err)
	}
	if _, err := store.Update(ctx, 2147483647, UpdateUserParams{Pseudo: "Missing"}); !errors.Is(err, ErrNotFound) {
		t.Fatalf("Update(missing) error = %v, want not found", err)
	}
	if err := store.ReplaceSkills(ctx, 2147483647, nil); !errors.Is(err, ErrNotFound) {
